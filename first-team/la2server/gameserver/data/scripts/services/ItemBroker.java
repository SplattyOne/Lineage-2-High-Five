...
public class ItemBroker extends Functions
{
	...
	public void list(String[] var)
	{
		...
		if(var.length != 5)
		{
			showBr("Некорректная длина данных", player, npc);
			return;
		}
		...
		{
			showBr("Некорректные данные", player, npc);
			return;
		}
		...
		if(allItems == null)
		{
			showBr("Ошибка - предметов такого типа не найдено", player, npc);
			return;
		}
		...
		out.append("</table>&nbsp;");
		showBr(out.toString(), player, npc);
	}
	...
	public void listForItem(String[] var)
	{
		...
		if(template == null)
		{
			showBr("Ошибка - itemId не определен.", player, npc);
			return;
		}
		...
		if(tmpItems == null)
		{
			showBr("Ошибка - такой тип предмета отсутствует.", player, npc);
			return;
		}
		...
		if(allItems == null)
		{
			showBr("Ошибка - предметов с таким названием не найдено.", player, npc);
			return;
		}
		...
		if(sortedItems == null)
		{
			showBr("Ошибка - ничего не найдено.", player, npc);
			return;
		}
	...
	public void path(String[] var)
	{
		...
		if(items == null)
		{
			showBr("Ошибка - предметов с таким именем не найдено.", player, npc);
			return;
		}
		...
		if(item == null)
		{
			showBr("Ошибка - предмет не найден.", player, npc);
			return;
		}
		...
		if (trader == null)
		{
			showBr("Торговец не найден, возможно он вышел из игры.", player, npc);
			return;
		}
		...
		{
			if (player.isLangRus())
				showBr("Внимание, цена или предмет изменились, будьте осторожны !", player, npc);
			else
				showBr("Caution, price or item was changed, please be careful !", player, npc);
		}
	...
	public void find(String[] var)
	{
		...
		{
			if (player.isLangRus())
				showBr("Пожалуйста введите от 1 до 16 символов.<br>[npc_%objectId%_Chat 0|<font color=\"FF9900\">Назад</font>]", player, npc);
			else
				showBr("Please enter from 1 up to 16 symbols.<br>[npc_%objectId%_Chat 0|<font color=\"FF9900\">Back</font>]", player, npc);
			return;
		}
		...
		if(allItems == null)
		{
			showBr("Ошибка - предметов с таким типом не найдено.", player, npc);
			return;
		}
